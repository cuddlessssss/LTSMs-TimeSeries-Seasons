...
trend_preds = {}
months_numeric = np.arange(len(pivot.index)).reshape(-1, 1)  # 0 to 23
...
for col_idx, (model_enc, account_enc) in enumerate(pivot.columns):
    ...
    reg = Lasso(alpha=0.001, positive=True)
...
wb.save('forecast_output_trend_seasonality_positive_learn_only.xlsx')
