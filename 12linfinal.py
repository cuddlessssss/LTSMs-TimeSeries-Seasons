...
future_total_raw = pd.read_excel('future_totaling.xlsx')
future_total_melted = future_total_raw.melt(
    id_vars='Model Code', var_name='year_month', value_name='total_sell_out'
)
future_total_melted['year_month'] = pd.to_datetime(future_total_melted['year_month']).dt.to_period('M')

# 🧼 Ensure 'Model Code' is treated as string type before applying string operations
...
data = pd.read_excel('your_past_data.xlsx', sheet_name='di_access')
data.columns = data.columns.str.strip()

# 🎯 Allow one or more categories
allowed_categories = ['DI AND UNIVERSAL ACCY']  # You can add more like ['PRO AUDIO', 'LCD TV']
data['Category Description'] = data['Category Description'].astype(str).str.strip().str.replace('\xa0', ' ', regex=False)
data = data[data['Category Description'].isin(allowed_categories)]
data = data[data['Dealer Net Sell Out Qty'] >= 0]
data['Date'] = pd.to_datetime(data['Date'], errors='coerce')
data['year_month'] = data['Date'].dt.to_period('M')
data = data[data['Date'] >= data['Date'].max() - pd.DateOffset(months=12)]

# 🧼 Clean whitespace and non-breaking spaces for 'Model Code' and 'Account Name'
...
data['Model Code_enc'] = f1_encoder.fit_transform(data['Model Code'])
data['Account Name_enc'] = f2_encoder.fit_transform(data['Account Name'])

# Use encoders on future totals
future_total_melted['model_enc'] = f1_encoder.transform(future_total_melted['Model Code'])

# 3. Preprocess Time Series Data
grouped = data.groupby(['year_month', 'Model Code_enc', 'Account Name_enc'])['Dealer Net Sell Out Qty'].sum().reset_index()
pivot = grouped.pivot_table(index='year_month', columns=['Model Code_enc', 'Account Name_enc'], values='Dealer Net Sell Out Qty', fill_value=0)
...
# 🔁 Convert encodings back to original strings
rescaled_df['Model Code'] = f1_encoder.inverse_transform(rescaled_df['model_enc'])
rescaled_df['Model Code'] = rescaled_df['Model Code'].astype(str).str.strip().str.replace('\xa0', ' ', regex=False)
rescaled_df['Model Code'] = rescaled_df['Model Code'].str.zfill(8)  # <-- Left pad with 0s to 8 characters
...
# 7. Excel Export
export_pivot = rescaled_df.pivot_table(index=['Model Code', 'Account Name'], columns='year_month', values='rounded_qty', fill_value=0)
highlight_pivot = rescaled_df.pivot_table(index=['Model Code', 'Account Name'], columns='year_month', values='highlight', aggfunc='first', fill_value='down')

wb = Workbook()
ws = wb.active
ws.title = "Forecasted Sell Out"

headers = ['Model Code', 'Account Name'] + [dt.strftime('%Y-%m') for dt in export_pivot.columns]
ws.append(headers)

fill_up = PatternFill(start_color='FFFF0000', end_color='FFFF0000', fill_type='solid')   # Red
fill_down = PatternFill(start_color='FF00FF00', end_color='FF00FF00', fill_type='solid') # Green
...
wb.save('forecast_model_code_di_padded.xlsx')
