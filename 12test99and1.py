...
# 4. Fit Linear Regression with and without trimming
...
for col_idx, (model_enc, account_enc) in enumerate(pivot.columns):
    ...
    reg_trimmed = LinearRegression().fit(X[mask], y[mask])
    pred_trimmed = reg_trimmed.predict(X)
    trend_preds_trimmed[(model_enc, account_enc)] = reg_trimmed.predict(X_future)

    reg_full = LinearRegression().fit(X, y)
    pred_full = reg_full.predict(X)
    trend_preds_full[(model_enc, account_enc)] = reg_full.predict(X_future)

    mae_list.append((model_enc, account_enc, mean_absolute_error(y, pred_trimmed), mean_absolute_error(y, pred_full)))
    mse_list.append((model_enc, account_enc, mean_squared_error(y, pred_trimmed), mean_squared_error(y, pred_full)))

# Compare Errors
eval_df = pd.DataFrame(mae_list, columns=['Model_enc', 'Account_enc', 'MAE_trimmed', 'MAE_full'])
eval_df['MSE_trimmed'] = [x[2] for x in mse_list]
eval_df['MSE_full'] = [x[3] for x in mse_list]

print("\n[INFO] Average Errors Comparison:")
print("Mean MAE (Trimmed):", eval_df['MAE_trimmed'].mean())
print("Mean MAE (Full):   ", eval_df['MAE_full'].mean())
print("Mean MSE (Trimmed):", eval_df['MSE_trimmed'].mean())
print("Mean MSE (Full):   ", eval_df['MSE_full'].mean())

# 5. Choose one forecast to continue with
forecast_preds = np.zeros((12, len(pivot.columns)))
for idx, key in enumerate(pivot.columns):
    forecast_preds[:, idx] = trend_preds_trimmed[key]  # Or switch to trend_preds_full[key]
...
wb.save('forecast_output_trend_seasonality_comparison.xlsx')
