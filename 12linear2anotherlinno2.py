...
future_total_raw = pd.read_excel('future_total_sell_outs_pivoted.xlsx', dtype={'Model Code': str})
future_total_raw.columns = future_total_raw.columns.str.strip()

if 'Model Name' in future_total_raw.columns and 'Model Code' not in future_total_raw.columns:
    future_total_raw.rename(columns={'Model Name': 'Model Code'}, inplace=True)

print("🧾 Columns in future_total_raw:", future_total_raw.columns.tolist())

if 'Model Code' not in future_total_raw.columns:
    raise ValueError("❌ 'Model Code' column not found in future_total_raw.")
...
data = pd.read_excel('your_past_data.xlsx', sheet_name='di', dtype={'Model Code': str})
data.columns = data.columns.str.strip()

print("📋 Columns in past data:", data.columns.tolist())

if 'Model Name' in data.columns and 'Model Code' not in data.columns:
    data.rename(columns={'Model Name': 'Model Code'}, inplace=True)

if 'Model Code' not in data.columns:
...
pivot = grouped.pivot_table(index='year_month', columns=['Model Code_enc', 'Account Name_enc'], values='Dealer Net Sell Out Qty', fill_value=0).sort_index(axis=1)
column_index = pivot.columns.to_list()

# 5. Trend + seasonality via LinearRegression
trend_preds = {}
...
for col_idx, (model_enc, account_enc) in enumerate(pivot.columns):
    ...
    future_month_dummies = pd.get_dummies(future_month_nums).reindex(columns=month_dummies.columns, fill_value=0)
...
wb.save('forecast_output_trend_seasonality_testing_actuals_02683812.xlsx')
