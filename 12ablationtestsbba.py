...
timestamps = pivot.index.to_timestamp()
months = timestamps.month.values
years = timestamps.year.values
month_sin = np.sin(2 * np.pi * months / 12)
month_cos = np.cos(2 * np.pi * months / 12)
time_index = np.arange(len(pivot))  # linearly increasing trend index

# Add more informative features
time_features = np.stack([month_sin, month_cos, years, months, time_index], axis=1)

# Use your existing variables: pivot, pivot_scaled, time_features, target_values
# These must be defined in advance based on your full pipeline.

# Parameters
input_len = 12
output_len = 12

# Define ablation feature selectors
def get_time_features(which='all'):
    month_sin = time_features[:, 0]
    month_cos = time_features[:, 1]
    years = time_features[:, 2]
    months = time_features[:, 3]
    time_index = time_features[:, 4]

    if which == 'all':
        return np.stack([month_sin, month_cos, years, months, time_index], axis=1)
    elif which == 'no_seasonality':
        return np.stack([years, months, time_index], axis=1)
    elif which == 'no_trend':
        return np.stack([month_sin, month_cos, years, months], axis=1)
    elif which == 'none':
        return np.empty((len(time_features), 0))
    else:
...
def train_variant(name, feature_type):
    ...
    if X_time.shape[-1] > 0:
        ...
        X_combined = np.transpose(X_seq, (0, 2, 1))  # (samples, models, timesteps)
        X_combined = np.transpose(X_combined, (0, 2, 1))  # (samples, timesteps, models)

    model = build_lstm_model(X_combined.shape[1:], y_target.shape[1:])
    history = model.fit(
        X_combined, y_target,
        epochs=100,
        batch_size=16,
    ...
    print(f"✅ {name} final validation loss: {final_loss:.6f}")
    return name, final_loss

# Run all 4 ablation variants
results = []
results.append(train_variant('With All Features', 'all'))
results.append(train_variant('No Seasonality (sin/cos)', 'no_seasonality'))
results.append(train_variant('No Trend (index)', 'no_trend'))
results.append(train_variant('No Time Features', 'none'))
