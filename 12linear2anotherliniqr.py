...
for col_idx, (model_enc, account_enc) in enumerate(pivot.columns):
    ...
    for idx in excluded_indices:
        print(f"🔍 Outlier removed (IQR) - Model={model_name}, Account={account_name}, "
              f"Month={pivot.index[idx]}, Qty={y[idx]:.2f}, outside range [{lower_bound:.2f}, {upper_bound:.2f}]")

    if mask.sum() >= 8:
        X_used, y_used = X_trend_season[mask], y[mask]
    else:
        X_used, y_used = X_trend_season, y

    reg = LinearRegression()
    reg.fit(X_used, y_used)
